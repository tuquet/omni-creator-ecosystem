-- ============================================================================
-- MODULE 3: ASYNCHRONOUS OUTBOX & WEBHOOKS QUEUE
-- Description: Transactional Outbox pattern & Outbound Webhook Subscriptions for event-driven async processing.
-- Usage: Execute in Supabase SQL Editor or append to migrations when async processing/webhooks are needed.
-- ============================================================================
...
CREATE OR REPLACE TRIGGER update_webhook_subscriptions_modtime
...
CREATE OR REPLACE TRIGGER trigger_log_project_event
...
CREATE OR REPLACE TRIGGER trigger_log_invitation_event
    AFTER INSERT ON public.tenant_invitations
    FOR EACH ROW EXECUTE FUNCTION public.log_invitation_event_to_outbox();

-- 6. Outbox Cleanup Helper Function (for pg_cron or periodic jobs)
