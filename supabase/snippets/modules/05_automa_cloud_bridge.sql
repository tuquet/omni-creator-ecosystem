-- ============================================================================
-- MODULE 5: AUTOMA CLOUD BRIDGE (DISTRIBUTED AUTOMATION ENGINE)
...
-- Note: All tables strictly adhere to the prefix: automa_*
-- Usage: Execute in Supabase SQL Editor or append to migrations when automa fleet is needed.
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_automa_workflows_deleted_at ON public.automa_workflows (tenant_id, deleted_at) WHERE deleted_at IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_automa_runners_tenant_status ON public.automa_runners (tenant_id, status);
CREATE INDEX IF NOT EXISTS idx_automa_runners_heartbeat ON public.automa_runners (status, last_heartbeat_at);
...
CREATE INDEX IF NOT EXISTS idx_automa_campaigns_workflow ON public.automa_campaign_runs (workflow_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_automa_campaigns_runner ON public.automa_campaign_runs (runner_id) WHERE runner_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_automa_logs_tenant_campaign ON public.automa_execution_logs (tenant_id, campaign_run_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_automa_logs_level ON public.automa_execution_logs (tenant_id, level, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_automa_schedules_tenant_active ON public.automa_schedules (tenant_id, is_active) WHERE is_active = TRUE;

-- 4. Triggers
DROP TRIGGER IF EXISTS set_automa_workflows_updated_at ON public.automa_workflows;
CREATE TRIGGER set_automa_workflows_updated_at BEFORE UPDATE ON public.automa_workflows FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();

DROP TRIGGER IF EXISTS set_automa_runners_updated_at ON public.automa_runners;
CREATE TRIGGER set_automa_runners_updated_at BEFORE UPDATE ON public.automa_runners FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();

DROP TRIGGER IF EXISTS set_automa_campaigns_updated_at ON public.automa_campaign_runs;
CREATE TRIGGER set_automa_campaigns_updated_at BEFORE UPDATE ON public.automa_campaign_runs FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();

DROP TRIGGER IF EXISTS set_automa_schedules_updated_at ON public.automa_schedules;
CREATE TRIGGER set_automa_schedules_updated_at BEFORE UPDATE ON public.automa_schedules FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
...
CROSS JOIN (
    VALUES 
        ...
        ('automa:logs:read')
) AS p(id) WHERE r.tenant_id IS NULL AND r.name IN ('owner', 'admin')
...
CROSS JOIN (
    VALUES 
        ('automa:workflows:read'), ('automa:runners:read'), ('automa:campaigns:read'),
        ('automa:campaigns:run'), ('automa:logs:read')
) AS p(id) WHERE r.tenant_id IS NULL AND r.name = 'member'
ON CONFLICT (role_id, permission_id) DO NOTHING;
...
CROSS JOIN (
    VALUES 
        ('automa:workflows:read'), ('automa:runners:read'), ('automa:campaigns:read'), ('automa:logs:read')
) AS p(id) WHERE r.tenant_id IS NULL AND r.name = 'viewer'
ON CONFLICT (role_id, permission_id) DO NOTHING;
...
CREATE POLICY "automa_workflows_select_active" ON public.automa_workflows
    ...
    USING (public.is_tenant_member(tenant_id) AND public.has_tenant_permission(tenant_id, 'automa:workflows:read') AND deleted_at IS NULL);
...
CREATE POLICY "automa_workflows_select_trash_admin" ON public.automa_workflows
    FOR SELECT TO authenticated
    USING (public.is_tenant_admin(tenant_id) AND deleted_at IS NOT NULL);
...
CREATE POLICY "automa_workflows_insert" ON public.automa_workflows
    ...
    WITH CHECK (public.is_tenant_member(tenant_id) AND public.has_tenant_permission(tenant_id, 'automa:workflows:manage'));
...
CREATE POLICY "automa_runners_select" ON public.automa_runners
    FOR SELECT TO authenticated
    USING (public.is_tenant_member(tenant_id) AND public.has_tenant_permission(tenant_id, 'automa:runners:read'));
...
CREATE POLICY "automa_campaigns_select" ON public.automa_campaign_runs
    FOR SELECT TO authenticated
    USING (public.is_tenant_member(tenant_id) AND public.has_tenant_permission(tenant_id, 'automa:campaigns:read'));
...
CREATE POLICY "automa_campaigns_insert" ON public.automa_campaign_runs
    FOR INSERT TO authenticated
    WITH CHECK (public.is_tenant_member(tenant_id) AND (public.has_tenant_permission(tenant_id, 'automa:campaigns:run') OR public.has_tenant_permission(tenant_id, 'automa:campaigns:manage')));
...
CREATE POLICY "automa_campaigns_update" ON public.automa_campaign_runs
    FOR UPDATE TO authenticated
    USING (public.has_tenant_permission(tenant_id, 'automa:campaigns:run') OR public.has_tenant_permission(tenant_id, 'automa:campaigns:manage'))
    WITH CHECK (public.has_tenant_permission(tenant_id, 'automa:campaigns:run') OR public.has_tenant_permission(tenant_id, 'automa:campaigns:manage'));
...
CREATE POLICY "automa_logs_select" ON public.automa_execution_logs
    FOR SELECT TO authenticated
    USING (public.is_tenant_member(tenant_id) AND public.has_tenant_permission(tenant_id, 'automa:logs:read'));
